...
def Reduce(r):
    ...
    for v in values:
        if v[0] == 'Orders':
            ...
            customers.append(v)
    if len(orders) == 0:
        orders.append(('Orders', ['null', 'null', 'null']))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: right_outer_join <file>", file=sys.stderr)
        exit(-1)

    spark = SparkSession \
        .builder \
        .appName("PythonRightOuterJoin") \
        .getOrCreate()
    ...
    right_outer_join = lines \
        .flatMap(Map) \
        .groupByKey() \
        .flatMap(Reduce)

    for i in right_outer_join.collect():
        print(i)
    spark.stop()
